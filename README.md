...
| Feature | Status |
|---|---|
| Manual trade logging (ticker, type, entry/exit, qty, strategy, notes) | 🔄 In Progress |
| Trade grade (A/B/C/D) — execution quality separate from P&L | 🔄 In Progress |
| Emotion tag (Confident / FOMO / Hesitant / Revenge / Bored / Patient) | 🔄 In Progress |
| Setup notes (pre-trade) + review notes (post-trade) | 🔄 In Progress |
| Market data auto-fill via Polygon.io/Alpaca + company logo via Clearbit | 📋 Planned |
| Multiple entries per position (adding to an open trade) | 🔄 In Progress |
| Trade log + detail views | 🔄 In Progress |
| Predefined strategy list | 🔄 In Progress |
| Offline-first local storage (SQLite) | 🔄 In Progress |
...
│   ├── (tabs)/             # Tab navigation
│   ├── trade/              # Trade detail + add screens
│   └── _layout.tsx
├── components/             # Reusable UI components
├── db/                     # SQLite schema + Drizzle config
├── hooks/                  # Custom React hooks
├── services/               # API integrations (market data)
...
**Phase 1 — MVP (Current)**
Offline trade logging. Core data model. Add trade form with auto-fill. Predefined strategy list. Trade grade + emotion tag. Trade log and detail views.
